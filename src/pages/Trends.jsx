  ...
  <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-5 mb-8">
    ...
    <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-5">
      {items.map((item, index) => (
        <Link
          key={index}
          to={`/products?search=${encodeURIComponent(item.label)}`}
          className="rounded-lg border border-gray-100 hover:shadow-md transition-all duration-300 bg-gray-50 hover:bg-white"
        >
          <img
            src={item.img}
            alt={item.label}
          ...
          />
          <p className="text-center py-2 text-gray-800 text-sm font-medium">
            {item.label}
          </p>
        </Link>
      ))}
